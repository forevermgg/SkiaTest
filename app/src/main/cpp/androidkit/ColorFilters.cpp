...
static jlong MakeMatrix(JNIEnv* env, jclass clazz, jfloatArray jcm) {
...
static jlong MakeHSLAMatrix(JNIEnv* env, jclass clazz, jfloatArray jcm) {
  SkASSERT(env->GetArrayLength(jcm) == 20);
...
static jlong MakeCompose(JNIEnv*, jclass clazz, jlong outer, jlong inner) {
  auto cf = SkColorFilters::Compose(
...
static void ColorFilter_Release(JNIEnv*, jclass clazz, jlong native_cf) {
...
int register_androidkit_HSLAMatrixColorFilter(JNIEnv* env) {
  ...
  const auto clazz =
      env->FindClass("org/skia/androidkit/HSLAMatrixColorFilter");
  return clazz ? env->RegisterNatives(clazz, methods, SK_ARRAY_COUNT(methods))
               : JNI_ERR;
}
