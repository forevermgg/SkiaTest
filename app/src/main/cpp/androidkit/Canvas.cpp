...
jint Canvas_GetWidth(JNIEnv* env, jclass clazz, jlong native_instance) {
  const auto* canvas = reinterpret_cast<const SkCanvas*>(native_instance);
  return canvas ? canvas->imageInfo().width() : 0;
}

jint Canvas_GetHeight(JNIEnv* env, jclass clazz, jlong native_instance) {
  const auto* canvas = reinterpret_cast<const SkCanvas*>(native_instance);
  return canvas ? canvas->imageInfo().height() : 0;
}

jint Canvas_Save(JNIEnv* env, jclass clazz, jlong native_instance) {
...
void Canvas_Restore(JNIEnv* env, jclass clazz, jlong native_instance) {
  if (auto* canvas = reinterpret_cast<SkCanvas*>(native_instance)) {
    canvas->restore();
  }
}

void Canvas_RestoreToCount(JNIEnv* env, jclass clazz, jlong native_instance,
                           jint count) {
  if (auto* canvas = reinterpret_cast<SkCanvas*>(native_instance)) {
...
jint Canvas_SaveLayer(JNIEnv* env, jclass clazz, jlong native_instance,
                      jlong native_paint) {
...
jlong Canvas_LocalToDevice(JNIEnv* env, jclass clazz, jlong native_instance) {
  if (auto* canvas = reinterpret_cast<SkCanvas*>(native_instance)) {
...
void Canvas_Concat(JNIEnv* env, jclass clazz, jlong native_instance,
                   jlong native_matrix) {
...
void Canvas_Concat16f(JNIEnv* env, jclass clazz, jlong native_instance,
                      jfloatArray jmatrix) {
...
void Canvas_Translate(JNIEnv* env, jclass clazz, jlong native_instance,
...
void Canvas_Scale(JNIEnv* env, jclass clazz, jlong native_instance, jfloat sx,
                  jfloat sy, jfloat sz) {
  if (auto* canvas = reinterpret_cast<SkCanvas*>(native_instance)) {
...
void Canvas_ClipPath(JNIEnv* env, jclass clazz, jlong native_instance,
                     jlong native_path, jint native_clipOp, jboolean doAA) {
  if (auto* canvas = reinterpret_cast<SkCanvas*>(native_instance)) {
...
void Canvas_ClipRect(JNIEnv* env, jclass clazz, jlong native_instance, jfloat l,
                     jfloat t, jfloat r, jfloat b, jint native_clipOp,
                     jboolean doAA) {
  if (auto* canvas = reinterpret_cast<SkCanvas*>(native_instance)) {
    canvas->clipRect(SkRect::MakeLTRB(l, t, r, b),
                     static_cast<SkClipOp>(native_clipOp), doAA);
  }
}

void Canvas_ClipRRect(JNIEnv* env, jclass clazz, jlong native_instance,
                      jfloat l, jfloat t, jfloat r, jfloat b, jfloat xRad,
                      jfloat yRad, jint native_clipOp, jboolean doAA) {
  if (auto* canvas = reinterpret_cast<SkCanvas*>(native_instance)) {
    canvas->clipRRect(
        SkRRect::MakeRectXY(SkRect::MakeLTRB(l, t, r, b), xRad, yRad),
        static_cast<SkClipOp>(native_clipOp), doAA);
  }
}

void Canvas_ClipShader(JNIEnv* env, jclass clazz, jlong native_instance,
                       jlong native_shader, jint native_clipOp) {
  if (auto* canvas = reinterpret_cast<SkCanvas*>(native_instance)) {
    if (auto* shader = reinterpret_cast<SkShader*>(native_shader)) {
      canvas->clipShader(sk_ref_sp(shader),
                         static_cast<SkClipOp>(native_clipOp));
    }
  }
}

void Canvas_DrawColor(JNIEnv* env, jclass clazz, jlong native_instance, float r,
...
void Canvas_DrawRect(JNIEnv* env, jclass clazz, jlong native_instance,
...
void Canvas_DrawImage(JNIEnv* env, jclass clazz, jlong native_instance,
                      jlong native_image, jfloat x, jfloat y,
                      jint sampling_desc, jfloat sampling_b,
                      jfloat sampling_c) {
  auto* canvas = reinterpret_cast<SkCanvas*>(native_instance);
  ...
  if (canvas && image) {
    canvas->drawImage(image, x, y,
                      androidkit::utils::SamplingOptions(
                          sampling_desc, sampling_b, sampling_c));
  }
}

void Canvas_DrawPath(JNIEnv* env, jclass clazz, jlong native_instance,
                     jlong native_path, jlong native_paint) {
  auto* canvas = reinterpret_cast<SkCanvas*>(native_instance);
...
int register_androidkit_Canvas(JNIEnv* env) {
  static const JNINativeMethod methods[] = {
      ...
      {"nRestoreToCount", "(JI)V",
       reinterpret_cast<void*>(Canvas_RestoreToCount)},
      {"nGetLocalToDevice", "(J)J",
       reinterpret_cast<void*>(Canvas_LocalToDevice)},
