...
jlong Image_Create(JNIEnv* env, jclass clazz, jbyteArray jdata) {
  auto size = env->GetArrayLength(jdata);
  auto* data = env->GetByteArrayElements(jdata, nullptr);
  auto image =
      SkImage::MakeFromEncoded(SkData::MakeWithCopy(data, SkToSizeT(size)));
...
void Image_Release(JNIEnv*, jclass clazz, jlong native_instance) {
  SkSafeUnref(reinterpret_cast<const SkImage*>(native_instance));
}

jint Image_GetWidth(JNIEnv*, jclass clazz, jlong native_instance) {
  const auto* image = reinterpret_cast<const SkImage*>(native_instance);
  return image ? image->width() : 0;
}

jint Image_GetHeight(JNIEnv*, jclass clazz, jlong native_instance) {
  const auto* image = reinterpret_cast<const SkImage*>(native_instance);
  return image ? image->height() : 0;
}

jlong Image_MakeShader(JNIEnv*, jclass clazz, jlong native_instance, jint jtmx,
                       jint jtmy, jint sampling_desc, jfloat sampling_b,
                       jfloat sampling_c, jlong native_matrix) {
  ...
  if (const auto* image = reinterpret_cast<const SkImage*>(native_instance)) {
    ...
    const auto sampling = androidkit::utils::SamplingOptions(
        sampling_desc, sampling_b, sampling_c);

    const auto* lm = reinterpret_cast<const SkM44*>(native_matrix);
    shader = lm ? image->makeShader(tmx, tmy, sampling, lm->asM33())
