 ...
 public:
  GradientData(JNIEnv* env, const jfloatArray& jcolors, const jfloatArray& jpos,
      ...
      : fEnv(env),
        ...
        fLocalMatrix(native_lm
                         ? reinterpret_cast<const SkM44*>(native_lm)->asM33()
                         : SkMatrix()) {
...
static jlong MakeLinear(JNIEnv* env, jclass clazz, jfloat x0, jfloat y0,
                        jfloat x1, jfloat y1, jfloatArray jcolors,
                        jfloatArray jpos, jint jtm, jlong native_lm) {
  const GradientData gdata(env, jcolors, jpos, jtm, native_lm);
  const SkPoint pts[] = {{x0, y0}, {x1, y1}};

  auto shader = SkGradientShader::MakeLinear(
      pts, gdata.colors(), nullptr, gdata.pos(), gdata.count(),
      gdata.tileMode(), 0, &gdata.localMatrix());

  return reinterpret_cast<jlong>(shader.release());
}

static jlong MakeRadial(JNIEnv* env, jclass clazz, jfloat x, jfloat y, jfloat r,
                        jfloatArray jcolors, jfloatArray jpos, jint jtm,
                        jlong native_lm) {
  const GradientData gdata(env, jcolors, jpos, jtm, native_lm);

  auto shader = SkGradientShader::MakeRadial(
      {x, y}, r, gdata.colors(), nullptr, gdata.pos(), gdata.count(),
      gdata.tileMode(), 0, &gdata.localMatrix());

  return reinterpret_cast<jlong>(shader.release());
}

static jlong MakeTwoPointConical(JNIEnv* env, jclass clazz, jfloat x0,
                                 jfloat y0, jfloat r0, jfloat x1, jfloat y1,
                                 jfloat r1, jfloatArray jcolors,
                                 jfloatArray jpos, jint jtm, jlong native_lm) {
  const GradientData gdata(env, jcolors, jpos, jtm, native_lm);

  auto shader = SkGradientShader::MakeTwoPointConical(
      {x0, y0}, r0, {x1, y1}, r1, gdata.colors(), nullptr, gdata.pos(),
      gdata.count(), gdata.tileMode(), 0, &gdata.localMatrix());

  return reinterpret_cast<jlong>(shader.release());
}

static jlong MakeSweep(JNIEnv* env, jclass clazz, jfloat x, jfloat y, jfloat sa,
                       jfloat ea, jfloatArray jcolors, jfloatArray jpos,
                       jint jtm, jlong native_lm) {
  const GradientData gdata(env, jcolors, jpos, jtm, native_lm);

  auto shader = SkGradientShader::MakeSweep(
      x, y, gdata.colors(), nullptr, gdata.pos(), gdata.count(),
      gdata.tileMode(), sa, ea, 0, &gdata.localMatrix());
...
int register_androidkit_TwoPointConicalGradient(JNIEnv* env) {
  static const JNINativeMethod methods[] = {
      {"nMakeTwoPointConical", "(FFFFFF[F[FIJ)J",
       reinterpret_cast<void*>(MakeTwoPointConical)},
  };

  const auto clazz =
      env->FindClass("org/skia/androidkit/TwoPointConicalGradient");
  return clazz ? env->RegisterNatives(clazz, methods, SK_ARRAY_COUNT(methods))
               : JNI_ERR;
}
