...
unordered_string_set split(const char* spacedList) {
...
EglManager::~EglManager() {
  if (hasEglContext()) {
    LOGW("~EglManager() leaked an EGL context");
  }
}
...
void EglManager::initialize() {
  ...
  if (mEglDisplay == EGL_NO_DISPLAY) {
    LOGE("Failed to get EGL_DEFAULT_DISPLAY! err=%s", eglErrorString());
  }

  EGLint major, minor;
  if (eglInitialize(mEglDisplay, &major, &minor) == EGL_FALSE)
    LOGE("Failed to initialize display %p! err=%s", mEglDisplay,
         eglErrorString());

  LOGV("Initialized EGL, version %d.%d", (int)major, (int)minor);
  ...
  if (enablePartialUpdates) {
    ...
    bool useBufferAge = true;
    if (useBufferAge && EglExtensions.bufferAge) {
  ...
  if (memcmp(&wideColorGamut, &SkNamedGamut::kDisplayP3,
             sizeof(wideColorGamut)) == 0) {
    hasWideColorSpaceExtension = EglExtensions.displayP3;
  } else if (memcmp(&wideColorGamut, &SkNamedGamut::kSRGB,
                    sizeof(wideColorGamut)) == 0) {
    hasWideColorSpaceExtension = EglExtensions.scRGB;
  } else {
    LOGF("Unsupported wide color space.");
  }
  mHasWideColorGamutSupport =
      EglExtensions.glColorSpace && hasWideColorSpaceExtension;

  auto* vendor = reinterpret_cast<const char*>(glGetString(GL_VENDOR));
  auto* version = reinterpret_cast<const char*>(glGetString(GL_VERSION));
  // Properties::enableRenderEffectCache =
  //    supportsRenderEffectCache(vendor, version);
  LOGV("RenderEffectCache supported %s on driver version %s", vendor, version);
}

EGLConfig EglManager::load8BitsConfig(EGLDisplay display,
                                      EglManager::SwapBehavior swapBehavior) {
...
EGLConfig EglManager::loadA8Config(EGLDisplay display,
  ...
  if (!eglChooseConfig(display, attribs, configs.data(), numConfigs,
                       &numConfigs)) {
    return EGL_NO_CONFIG_KHR;
  }
...
void EglManager::initExtensions() {
  ...
  EglExtensions.displayP3 =
      extensions.has("EGL_EXT_gl_colorspace_display_p3_passthrough");
  EglExtensions.hdr = extensions.has("EGL_EXT_gl_colorspace_bt2020_pq");
  EglExtensions.contextPriority = extensions.has("EGL_IMG_context_priority");
  EglExtensions.surfacelessContext =
      extensions.has("EGL_KHR_surfaceless_context");
  EglExtensions.fenceSync = extensions.has("EGL_KHR_fence_sync");
  EglExtensions.waitSync = extensions.has("EGL_KHR_wait_sync");
  EglExtensions.nativeFenceSync =
      extensions.has("EGL_ANDROID_native_fence_sync");
}

bool EglManager::hasEglContext() { return mEglDisplay != EGL_NO_DISPLAY; }
...
void EglManager::loadConfigs() {
  ...
  if (mEglConfig == EGL_NO_CONFIG_KHR) {
    if (mSwapBehavior == SwapBehavior::Preserved) {
      ...
      LOGF("Failed to choose config, error = %s", eglErrorString());
    }
  }
  ...
  if (EglExtensions.pixelFormatFloat) {
    ...
    if (mEglConfigF16 == EGL_NO_CONFIG_KHR) {
      LOGE(
          "Device claims wide gamut support, cannot find matching config, "
          "error = %s",
          eglErrorString());
  ...
  if (mEglConfig1010102 == EGL_NO_CONFIG_KHR) {
    LOGW("Failed to initialize 101010-2 format, error = %s", eglErrorString());
  }
}
...
void EglManager::createContext() {
  ...
  if (mEglContext == EGL_NO_CONTEXT) {
    LOGE("Failed to create context, error = %s", eglErrorString());
  }
}

void EglManager::createPBufferSurface() {
  if (mEglDisplay == EGL_NO_DISPLAY) {
    LOGE("usePBufferSurface() called on uninitialized GlobalContext!");
  }
...
Result<EGLSurface, EGLint> EglManager::createSurface(
    EGLNativeWindowType window, ColorMode colorMode,
    sk_sp<SkColorSpace> colorSpace) {
  if (!hasEglContext()) {
    LOGF("Not initialized");
  }

  if (!EglExtensions.noConfigContext) {
    // The caller shouldn't use A8 if we cannot switch modes.
    if (colorMode == ColorMode::A8) {
      LOGF("Cannot use A8 without EGL_KHR_no_config_context!");
    }
  ...
  // color gamut rendering is off, the app simply renders in the display's
  // native color gamut.
  //
  ...
  if (colorMode == ColorMode::A8) {
    ...
    if (!mEglConfigA8) {
      ...
      if (!mEglConfigA8) {
        LOGF("Requested ColorMode::A8, but EGL lacks support! error = %s",
             eglErrorString());
      }
    }
    config = mEglConfigA8;
    ...
    if (EglExtensions.glColorSpace) {
      ...
      switch (colorMode) {
        ...
        case ColorMode::WideColorGamut: {
          ...
          if (!colorSpace->toXYZD50(&colorGamut)) {
            LOGF("Could not get gamut matrix from color space");
          }

          if (memcmp(&colorGamut, &SkNamedGamut::kDisplayP3,
                     sizeof(colorGamut)) == 0) {
            attribs[1] = EGL_GL_COLORSPACE_DISPLAY_P3_PASSTHROUGH_EXT;
          } else if (memcmp(&colorGamut, &SkNamedGamut::kSRGB,
                            sizeof(colorGamut)) == 0) {
            attribs[1] = EGL_GL_COLORSPACE_SCRGB_EXT;
          } else if (memcmp(&colorGamut, &SkNamedGamut::kRec2020,
                            sizeof(colorGamut)) == 0) {
            attribs[1] = EGL_GL_COLORSPACE_BT2020_PQ_EXT;
          } else {
            LOGF("Unreachable: unsupported wide color space.");
          }
          break;
        }
        case ColorMode::Hdr:
        ...
        case ColorMode::Hdr10:
          ...
          break;
        case ColorMode::A8:
          LOGF("Unreachable: A8 doesn't use a color space");
          break;
      }
    }
  }
  ...
  EGLSurface surface =
      eglCreateWindowSurface(mEglDisplay, config, window, attribs);
  if (surface == EGL_NO_SURFACE) {
    return Error<EGLint>{eglGetError()};
  }

  if (mSwapBehavior != SwapBehavior::Preserved) {
    if (eglSurfaceAttrib(mEglDisplay, surface, EGL_SWAP_BEHAVIOR,
...
void EglManager::destroySurface(EGLSurface surface) {
  ...
  if (!eglDestroySurface(mEglDisplay, surface)) {
    LOGW("Failed to destroy surface %p, error=%s", (void*)surface,
         eglErrorString());
  }
}
...
bool EglManager::makeCurrent(EGLSurface surface, EGLint* errOut, bool force) {
  ...
  if (!eglMakeCurrent(mEglDisplay, surface, surface, mEglContext)) {
    if (errOut) {
      *errOut = eglGetError();
      LOGW("Failed to make current on surface %p, error=%s", (void*)surface,
           egl_error_str(*errOut));
    } else {
      LOGF("Failed to make current on surface %p, error=%s", (void*)surface,
           eglErrorString());
    }
  }
...
Frame EglManager::beginFrame(EGLSurface surface) {
  if (surface == EGL_NO_SURFACE) {
    LOGF("Tried to beginFrame on EGL_NO_SURFACE!");
  }
  makeCurrent(surface);
  Frame frame;
...
void EglManager::damageFrame(const Frame& frame, const SkRect& dirty) {
  ...
  if (EglExtensions.setDamage && mSwapBehavior == SwapBehavior::BufferAge) {
    ...
    // LOGF("Failed to set damage region on surface %p, error=%s",
...
bool EglManager::swapBuffers(const Frame& frame, const SkRect& screenDirty) {
  ...
  if (err == EGL_BAD_SURFACE || err == EGL_BAD_NATIVE_WINDOW) {
    ...
    LOGW("swapBuffers encountered EGL error %d on %p, halting rendering...",
         err, frame.mSurface);
    return false;
  }
  LOGF("Encountered EGL error %d %s during rendering", err, egl_error_str(err));
...
void EglManager::fence() {
  ...
  eglClientWaitSyncKHR(mEglDisplay, fence, EGL_SYNC_FLUSH_COMMANDS_BIT_KHR,
                       EGL_FOREVER_KHR);
...
bool EglManager::setPreserveBuffer(EGLSurface surface, bool preserve) {
  ...
  if (!preserved) {
    LOGW("Failed to set EGL_SWAP_BEHAVIOR on surface %p, error=%s",
         (void*)surface, eglErrorString());
    // Maybe it's already set?
    EGLint swapBehavior;
    if (eglQuerySurface(mEglDisplay, surface, EGL_SWAP_BEHAVIOR,
                        &swapBehavior)) {
      preserved = (swapBehavior == EGL_BUFFER_PRESERVED);
    } else {
      LOGW("Failed to query EGL_SWAP_BEHAVIOR on surface %p, error=%p",
           (void*)surface, eglErrorString());
    }
  }
