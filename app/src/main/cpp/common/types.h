...
enum class PixelFormat : int32_t {
   ...
   *
   * In an image buffer with this format, starting from the first pixel of each
   * row, each two consecutive pixels are packed into 3 bytes (24 bits). The
   * first and second byte contains the top 8 bits of first and second pixel.
   * The third byte contains the 4 least significant bits of the two pixels, the
   * exact layout data for each two consecutive pixels is illustrated below
   ...
   * the end of each row, the entire image data is densely packed. When stride
   * is larger than width * (12 / 8), padding bytes will be present at the end
   * of each row (including the last row).
   *
   * This format must be accepted by the allocator when used with the
   * following usage flags:
